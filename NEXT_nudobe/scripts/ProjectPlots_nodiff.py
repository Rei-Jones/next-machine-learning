...
file_identify = f"{event_type}_{pressure}_nodiff"
...
def get_data(XYZC, vertex, eid, file_identify, split):
    ...
    for dim, X, Y, vx, vy in projections:
        ...
        # cast to native python floats here so downstream json.dumps never
        # chokes on numpy float32/float64 scalars sneaking in via vx/vy
        axis_limits[dim] = {
...
for eid, data in part_df.groupby("event_id"):
    ...
    split = get_split()
    (XYZC, vertex) = PlotEvent3D(hits_df, part_df, eid)
    x, y, z, axis_limits = get_data(XYZC, vertex, eid, file_identify, split=split)
    if x is None:
        continue
    # numpy scalar type slips through besides the ones already cast above
    all_limits.append({"event_id": eid, "axis_limits": json.dumps(axis_limits, default=numpy_safe)})
