...
python3 ProjectPlotsNew.py --input_file ${EVENT_TYPE}_nexus_${JOBID}.h5 --pressure "$PRESSURE" --diffusion "nodiff"  --type "${EVENT_TYPE}"
python3 ProjectPlotsNew.py --input_file ${EVENT_TYPE}_0.1percent_smear_${JOBID}.h5 --pressure "$PRESSURE" --diffusion "0.1percent"  --type "${EVENT_TYPE}"
python3 ProjectPlotsNew.py --input_file ${EVENT_TYPE}_5.0percent_smear_${JOBID}.h5 --pressure "$PRESSURE" --diffusion "5.0percent"  --type "${EVENT_TYPE}"

python3 get_angle_data.py ${EVENT_TYPE}_nexus_${JOBID}.h5 $PRESSURE "nodiff" ${EVENT_TYPE}_${PRESSURE}_nodiff_limits.jsonl
python3 get_angle_data.py ${EVENT_TYPE}_0.1percent_smear_${JOBID}.h5 $PRESSURE "0.1percent" ${EVENT_TYPE}_${PRESSURE}_nodiff_limits.jsonl #the limits are the same for all diffusions
python3 get_angle_data.py ${EVENT_TYPE}_5.0percent_smear_${JOBID}.h5 $PRESSURE "5.0percent" ${EVENT_TYPE}_${PRESSURE}_nodiff_limits.jsonl
